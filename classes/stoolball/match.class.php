...
class Match implements IHasShortUrl
{
	...
	public function GetSearchDescription() 
	{
		...
		if ($this->GetMatchType() == MatchType::TOURNAMENT_MATCH)
		{
			$o_tourney = $this->GetTournament();
			if (is_object($o_tourney))
			{
				...
				if (is_object($this->GetGround())) $description .= " at " . $this->GetGround()->GetNameAndTown();
				$description .= ".";
			}
		}
		else
		{
			$description = ucfirst(MatchType::Text($this->GetMatchType()));
			if (is_object($this->GetGround())) $description .= " at " . $this->GetGround()->GetNameAndTown();
