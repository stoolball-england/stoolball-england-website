...
CREATE TABLE IF NOT EXISTS `nsa_forum_topic` (
  ...
  `total_messages` INT(4) UNSIGNED NOT NULL DEFAULT '0',
  `status` ENUM('open', 'closed', 'moved') NOT NULL DEFAULT 'open',
  PRIMARY KEY (`id`),
  INDEX `category_id` (`category_id` ASC, `first_message_id` ASC, `last_message_id` ASC))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `nsa_user` (
  ...
  `search_page_size` INT(3) UNSIGNED NOT NULL DEFAULT '10',
  `topics_page_size` INT(3) UNSIGNED NOT NULL DEFAULT '50',
  `messages_page_size` INT(3) UNSIGNED NOT NULL DEFAULT '30',
